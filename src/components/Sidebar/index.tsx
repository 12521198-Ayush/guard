...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <>
      ...
      <aside
        ref={sidebar}
        className={`absolute left-0 top-0 z-9999 flex h-screen w-82.5 flex-col overflow-y-hidden bg-black shadow-xl rounded-tr-3xl duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        {/* SIDEBAR HEADER */}
              ...
              <ul className="mb-6 flex flex-col gap-2">
                <li>
                  <Link
                    ...
                    className={`group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${pathname === "/menu" && "text-white"}`}
                  >
